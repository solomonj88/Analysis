...
for (ai in assay_list) {
  ...
  # Specimen type breakdown for this assay (all tests regardless of result)
  ...
  # Assay header row — blue + bold in display, no data values
  t3_rows[[length(t3_rows)+1]] <- tibble(
    ...
    `DNA Used (ng) Mean (SD)`                = "",
    `Mean EXON 21 WT Droplets Median [IQR]`  = "",
    `Mean EXON 10 WT Droplets Median [IQR]`  = "",
    `Specimen Type: Blood n (%)`             = "",
    `Specimen Type: Cyst Fluid n (%)`        = "",
    `Specimen Type: Other n (%)`             = ""
  )

  # Positive and Negative data rows
  for (res in c("Positive", "Negative")) {
    ...
    # Fluid vol: unique sample_id_for_di_dataset within this result group, exclude zeros/blanks
    ...
    t3_rows[[length(t3_rows)+1]] <- tibble(
      ...
      `Fluid Vol (mL) Median [IQR]`            = iqr_s(s_vol$vol_val),
      `DNA Conc (ng/µL) Median [IQR]`         = iqr_s(s$dna_conc_val),
      `DNA Used (ng) Mean (SD)`                = msd_s(s$dna_used_val),
      `Mean EXON 21 WT Droplets Median [IQR]`  = if(ai$ex) iqr_s(s$ex21_val) else "N/A",
      `Mean EXON 10 WT Droplets Median [IQR]`  = if(ai$ex) iqr_s(s$ex10_val) else "N/A",
      `Specimen Type: Blood n (%)`             = fmt_np(n_bl,  pct_bl),
      `Specimen Type: Cyst Fluid n (%)`        = fmt_np(n_ly,  pct_ly),
      `Specimen Type: Other n (%)`             = fmt_np(n_oth, pct_oth)
    )
  }
}
...
gt_t3 <- table3_df |>
  ...
  tab_source_note("IQR = interquartile range. SD = standard deviation. n (%) is proportion of that assay's valid tests. Mean EXON 21/10 WT droplet counts reported as Median [IQR]; N/A for TEK and BRAF assays. Specimen type breakdown reflects all valid tests for that assay regardless of result.") |>
